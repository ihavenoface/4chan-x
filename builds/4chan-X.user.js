...
(function() {
  ...
  Header = {
    ...
    generateBoardList: function(text) {
      ...
      nodes = text.match(/[\w@]+(-(all|title|replace|full|index|catalog|text:"[^"]+"))*|break|[^\w@]+/g).map(function(t) {
  ...
  YourPosts = {
    init: function() {
      if (g.VIEW === 'catalog' || !Conf['Mark own Posts'] || !Conf['Quick Reply']) {
        return;
      }
      return Post.prototype.callbacks.push({
        name: 'Mark own Posts',
    ...
    node: function() {
      if (QR.db.get({
        ...
        return $.addClass(this.nodes.root, 'your-post');
      }
    }
  };
  ...
  ThreadUpdater = {
    ...
    getInterval: function() {
      return ThreadUpdater.seconds = ThreadUpdater.interval;
    },
  ...
  Linkify = {
    ...
    seek: function(node, uid) {
      ...
      if (!(next = (inSpoiler || node).nextSibling)) {
        return;
      }
  ...
  IDColor = {
    ...
    node: function() {
      ...
      span.firstElementChild.style.cssText = "background-color: rgb(" + rgb[0] + "," + rgb[1] + "," + rgb[2] + ");\ncolor: " + (rgb[3] ? 'black' : 'white') + ";";
      $.addClass(span, 'painted');
      if (text = span.firstChild) {
        $.rm(text);
      }
      if (text = span.lastChild) {
        return $.rm(text);
      }
    },
  ...
  Main = {
    ...
    initReady: function() {
      var board, boardChild, el, err, errors, href, posts, thread, threadChild, threads, _i, _j, _len, _len1, _ref, _ref1;
